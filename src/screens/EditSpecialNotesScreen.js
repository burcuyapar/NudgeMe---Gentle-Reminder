...
const EditSpecialNotesScreen = ({ navigation, route }) => {
  ...
  const handleSave = async () => {
    setIsLoading(true);
    const result = await saveFamilyData(childrenData);
    setIsLoading(false);
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Edit Special Notes</Text>
        <View style={{ width: 40 }} />
      </View>
          ...
          <View style={styles.card}>
            <View style={styles.formGroup}>
              ...
              <TextInput
                ...
                placeholder="e.g. Vitamin D drop in mornings..."
                value={child.special_notes}
                onChangeText={(text) => updateChild('special_notes', text)}
                multiline
                numberOfLines={6}
                autoFocus={true}
              />
...
export default EditSpecialNotesScreen;
